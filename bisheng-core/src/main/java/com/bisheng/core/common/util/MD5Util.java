...
public class MD5Util {
    ...
    //加的盐
    private static final String SALT = "HXWcjvQWVG0wI3FQBLZpQ3pWj48AV63d";
   
    public static String encode(String inbuf) {
          md5Init();
          byte[] inbufBytes = null;
          try {
        	  inbufBytes = inbuf.getBytes("UTF-8");
          } catch (Exception e) {
        	  
          }
          md5Update(inbufBytes, inbufBytes.length);
          md5Final();
          digestHexStr = "";
          for (int i = 0; i < 16; i++) {
     ...
     * @return
     * @author 柏松
     * @created 2013-1-23 下午3:01:20
     */
    public static String encodeWithSalt(String src) {
        return encode(encode(src)+SALT);
    }
    ...
    private static void md5Memcpy (byte[] output, byte[] input,
    ...
    {
        System.out.println(MD5Util.encodeWithSalt("password"));
    }
}